...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = (bce_loss(tf.ones(N),logits_real))+(bce_loss(tf.zeros(N),logits_fake))
...
def generator_loss(logits_fake):
    ...
    loss = (bce_loss(tf.ones(N),logits_fake))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 0.5 * tf.math.reduce_mean((scores_real-tf.ones(N))**2) + \
            0.5 * tf.math.reduce_mean((scores_fake)**2)
...
def ls_generator_loss(scores_fake):
    ...
    loss = 0.5 * tf.math.reduce_mean((scores_fake-tf.ones(N))**2)
