...
def compute_saliency_maps(X, y, model):
    ...
    #forward pass 
    scores = model(X)
    scores = (scores.gather(1, y.view(-1, 1)).squeeze()) # correct scores for each image - (5,)
    
    #backward pass - parameter is [1,1,1,1,1]
    scores.backward(torch.FloatTensor([1.0]*scores.shape[0]))
    
    #saliency - max over all three channels
...
def make_fooling_image(X, target_y, model):
    ...
    # keep performing gradient ascent until we fool all
    while True:
        # get prediction
        scores = model(X_fooling)
        _, idx = torch.max(scores, axis=1)

        # if the image still isn't fooled
        if (idx != target_y):
            # backward pass from the target score
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    # forward & backward pass for score with reg
    scores = model(img)
