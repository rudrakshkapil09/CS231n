...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        fc1, (fc1_cache, relu1_cache) = affine_relu_forward(X, W1, b1)
        fc2, fc2_cache = affine_forward(fc1, W2, b2)
        scores = fc2
        ...
        dfc1, dW2, db2 = affine_backward(dfc2, fc2_cache)
        dX, dW1, db1 = affine_relu_backward(dfc1, (fc1_cache, relu1_cache))
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            self.params['W'+str(i+1)] = np.random.normal(loc=0, scale=weight_scale, size = (layers_dims[i], layers_dims[i+1]))
    ...
    def loss(self, X, y=None):
        ...
        # input through all H
        gamma,beta,bn_params,do_params = None,None,None,None
        for i in range(self.num_layers-1):
            ...
            x, cache = self.affine_bn_relu_dropout_forward(x,w,b,gamma,beta,bn_params)
            caches.append(cache)
        ...
        # first output
        dx,dw,db = affine_backward(dout, caches[self.num_layers-1])
        w = self.params['W'+str(self.num_layers)]
        ...
        for i in reversed(range(self.num_layers-1)):
            dx,dw,db,dgamma,dbeta = self.affine_bn_relu_dropout_backward(dx,caches[i])
            w = self.params['W'+str(i+1)]
    ...
    def affine_bn_relu_dropout_forward(self, x, w, b, gamma, beta, bn_param):
