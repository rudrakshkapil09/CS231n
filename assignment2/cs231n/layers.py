...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N): # each data 
        for f in range(F): # each filter
            for j in range(H_prime): # height of op
                for i in range(W_prime): # width of op
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        for f in range(F):
            db[f] += dout[n, f].sum() # db is sum across all filters
...
def max_pool_forward_naive(x, pool_param):
    ...
    # Naive Loops
    for n in range(N): # for each data item
        for j in range(H_prime):
            for i in range(W_prime):
                # get the maximum of window along heightand width
...
def max_pool_backward_naive(dout, cache):
    ...
    # Naive Loops
    for n in range(N): # for each data item
        for c in range(C): # for each of the depths 
            for j in range(H_prime): 
                for i in range(W_prime):
                    # ind is maximum of window in depth c -> across flattened 
                    ind = np.argmax(x[n, c, j*stride:j*stride+HH, i*stride:i*stride+WW])
                    
                    # ind1 and ind2 are coordinates of ind in the window 
                    ind1, ind2 = np.unravel_index(ind, (HH, WW))
                    
                    # dx at that max position is only given the entire dout
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    x = x.transpose(0,2,3,1).reshape(N*H*W, C) # height width then depth for each
    out, cache = batchnorm_forward(x, gamma, beta, bn_param) # batchnorm it
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    N, C, H, W = x.shape
    size = (N*G, C//G *H*W) # reshape into n groups of size (C/G) * H * W
    x = x.reshape(size).T
    gamma = gamma.reshape(1, C, 1, 1) # add the extra dimensions
    beta = beta.reshape(1, C, 1, 1)   # add the extra dimensions
